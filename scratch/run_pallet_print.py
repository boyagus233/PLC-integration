...
def trigger_pallet_print(line_no):
    url_pallet = "https://api.pms.yuasa.seavihive.com/api/fix-scanner-pallet"
    payload = {"line_no": line_no}

    print(f"\nHitting Pallet API for Line {line_no}...")
    try:
        res = requests.post(url_pallet, json=payload, timeout=10)
...
success = trigger_pallet_print("14")
if not success:
    trigger_pallet_print("1")
